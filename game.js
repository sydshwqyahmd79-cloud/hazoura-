...
// 1. نحمل الصور الاول - بالاسماء الصح
...
function shoot() { 
    if(gameReady) arrows.push({ x: archer.x + 90, y: archer.y + 50, w: 50, h: 12, speed: 15 }); 
}
...
function createBlood(x, y) { 
    for(let i = 0; i < 15; i++) particles.push({x, y, vx: Math.random()*4-2, vy: Math.random()*4-2, life: 20}) 
}
...
function update() {
    ...
    monsters.forEach(m => { 
        if(m.x < archer.x + 100 && m.x + m.w > archer.x) archer.hp -= 0.5; 
    });
}
